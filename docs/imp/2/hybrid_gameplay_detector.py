...
class HybridGameplayDetector:
    ...
    def __init__(
        ...
        self.frame_variances = deque(maxlen=window_size)
    ...
    def update(
        ...
        frame_mean = np.mean(frame.astype(np.float32))
        ...
        return {
            ...
            'signals': {
                'variance': {
                    ...
                    'current': float(np.mean(self.frame_variances)) if self.frame_variances else 0.0,
...
def integration_example():
    """
    Example of how to integrate into worker main loop.
    """
    
    # 1. Initialize detector at worker startup
    gameplay_detector = HybridGameplayDetector()
    
    # 2. In rollout loop, update every frame
    # (This is pseudocode - adapt to your actual worker structure)
    
    while True:
        # Get current frame
        frame = get_current_frame()  # Returns numpy array
        
        # Execute action
        action = select_action()
        execute_action(action)
        
        # Check if meaningful step occurred
        step_occurred = check_if_step_occurred()  # Returns bool
        
        # Update detector
        gameplay_started = gameplay_detector.update(
            frame=frame,
            step_increment=1 if step_occurred else 0,
            action_taken=True  # Action was executed
        )
        
        # Use in status
        worker_status = {
            'gameplay_started': gameplay_started,
            'gameplay_confidence': gameplay_detector.confidence,
            # ... other status fields
        }
...
def test_detector():
    ...
    """
    print("=== Testing HybridGameplayDetector ===")
    print()
    
    # Test 1: Menu scenario
    print("Test 1: Menu Detection")
    print("-" * 60)
    
    detector = HybridGameplayDetector()
    
    # Simulate menu: static frame with small noise
    menu_frame = np.ones((720, 1280, 3)) * 128
    
    for i in range(180):  # 3 seconds
        frame = menu_frame + np.random.randn(720, 1280, 3) * 5
        
        # Actions but no steps in menu
        ...
        if i % 60 == 0:
            print(f"  {i/60:.0f}s: gameplay={gameplay}, confidence={detector.confidence:.3f}")
    
    print(f"Result: {'✅ PASS' if not gameplay else '❌ FAIL (false positive)'}")
    print()
    
    # Test 2: Gameplay scenario
    print("Test 2: Gameplay Detection")
    print("-" * 60)
    
    detector = HybridGameplayDetector()
    
    for i in range(240):  # 4 seconds
        # Dynamic frame (gameplay)
        frame = np.random.randint(0, 255, (720, 1280, 3), dtype=np.uint8)
        
        # Actions lead to steps
        action_taken = (i % 5 == 0)
        step_increment = 1 if action_taken else 0
        ...
        if gameplay:
            print(f"  ✅ Detected at {i/60:.1f}s")
            break
    
    print(f"Result: {'✅ PASS' if gameplay else '❌ FAIL (missed detection)'}")
    print()
